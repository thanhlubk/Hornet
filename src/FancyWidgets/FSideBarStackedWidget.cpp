...
void FSideBarStackedWidget::updateStyleCurrentPage()
{
    ...
    for (QObject* obj : pCurrentWidget->children()) 
    {
        ...
        else if (FSeparator* pSeparator = qobject_cast<FSeparator*>(obj))
        {
        ...
        else if (FListWidgetScrollAware *pListContent = qobject_cast<FListWidgetScrollAware *>(obj))
        {
            pListContent->setSpacing(getDisplaySize(4).toInt() / 2);
        }
    }
...
QWidget* FSideBarStackedWidget::addPageJson(const QJsonArray& jsonArray, const QString& title)
{
    auto pListContent = new FListWidgetScrollAware();
    pListContent->enableSpacingBorder(false);
    ...
    for (const QJsonValue& arrayVal : jsonArray) 
    {
        ...
        GET_JSON_VALUE_STRING(jsonObj, strMode, mode)

        QMetaEnum metaEnum = QMetaEnum::fromType<FPanel::ContentMode>();
        bool ok;
        auto enumValue = static_cast<FPanel::ContentMode>(metaEnum.keyToValue(strMode.toUtf8().constData(), &ok));
        ...
        auto menu = new FPanel(enumValue, pListContent);
        ...
        pListContent->addWidget(menu);
        count++;
    }

    QWidget *page = createPageLayout(title, pListContent);
    
    return page;
}

QWidget *FSideBarStackedWidget::addPage(PageFactory factory, const QString &title)
{
    QWidget *widget = factory();
    if (!widget)
        return nullptr;

    QWidget *page = createPageLayout(title, widget);
    return page;
}
...
QWidget* FSideBarStackedWidget::createPageLayout(const QString &title, QWidget *widget)
{
    ...
    auto pSeparator = new FSeparator(Qt::Horizontal, page);
